...
setwd("G:/SpinalCord/Publication/RawExpression/4Covar_NovaSeq")
...
write.csv(sig1,"NOVASEQ_SpinalCord_ALSCohort_4Covar_SexSigGenes_9-15-23.csv")

# vsd = varianceStabilizingTransformation(dseq)
# vstcounts = assay(vsd)

##########################################################################
#As expected, RIN dependent expression is severe. Full removal of all sig genes may dramatically reduce subtype signal. Try to balance. 

#Outlier Test
#Results: removing centered RIN values less than -2 gives roughly the same number of sig genes (17545 --> 17137)
...
# 
# FullPheno2 = FullPheno[-which(FullPheno$RIN < -2),]
# rALSCountData_RIN2 = rALSCountData_RIN[,-which(FullPheno$RIN < -2)]
# 
# dds = DESeqDataSetFromMatrix(countData = rALSCountData_RIN2, colData = FullPheno2, design= ~ RIN, tidy=F)
# dseq = DESeq(dds,betaPrior=T)
# res = results(dseq)
# sig2 = res[! is.na(res$padj) & res$padj<0.05,]
# hist(sig2$padj,breaks = 50)

#Same threshold applied to HiSeq cohort to keep consistent - despite less severe effects due to RIN. 
...
####################
#Test 
#Result: VST normalized expression with Sex + RIN vs RIN only design equation are equivalent. 
# dds = DESeqDataSetFromMatrix(countData = rALSCountData_RIN, colData = FullPheno, design= ~ Sex + RIN, tidy=F)
# dseq = DESeq(dds,betaPrior=T)
# res = results(dseq)
# sig3 = res[! is.na(res$padj) & res$padj<0.05,]
# 
# vsd = varianceStabilizingTransformation(dseq)
# vstcounts3 = assay(vsd)
# 
# table(sig %in% rownames(sig3))
####################

dim(vstcounts);dim(filtdata)

file = "NOVASEQ_SpinalCord_ALSCohort_4Covariates_VST_Gene_TE_9-15-23.csv"
write.csv(filtdata,file)
file = "NOVASEQ_SpinalCord_ALSCohort_4Covariates_VST_Gene_TE_9-15-23.txt"
...
file = "NOVASEQ_SpinalCord_ALSCohort_4Covariates_MAD10k_GeneENSEMBL_TE_9-15-23.txt"
write.table(mad10000,file,sep="\t",quote=F,row.names=T,col.names=T)
file = "NOVASEQ_SpinalCord_ALSCohort_4Covariates_MAD10k_GeneENSEMBL_TE_9-15-23.csv"
...
file = "NOVASEQ_SpinalCord_ALSCohort_4Covariates_MAD5k_GeneENSEMBL_TE_9-15-23.txt"
write.table(mad5000,file,sep="\t",quote=F,row.names=T,col.names=T)
file = "NOVASEQ_SpinalCord_ALSCohort_4Covariates_MAD5k_GeneENSEMBL_TE_9-15-23.csv"
...
file = "NOVASEQ_SpinalCord_ALSCohort_MAD10k_4Covariates_GeneSYMBOL_TE_9-15-23.csv"
...
file = "NOVASEQ_SpinalCord_ALSCohort_MAD5k_4Covariates_GeneSYMBOL_TE_9-15-23.csv"
