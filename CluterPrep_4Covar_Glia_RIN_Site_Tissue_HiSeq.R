...
SiteMeta = Meta[Meta$sample_id_alt %in% rownames(coldata2),]
...
########################################################################################################################################
#############################################  PART 4: REMOVE OLIGO MARKER GENES   #####################################################################
########################################################################################################################################
...
setwd("G:/SpinalCord/Publication/RawExpression/4Covar_HiSeq")
...
write.csv(sig1,"HISEQ_SpinalCord_ALSCohort_4Covar_SexSigGenes_9-15-23.csv")

#No samples with missing RIN (incomplete design equation)
##########################################################################
#Approach: Lower padj threshold, weakly sig genes may not cause problems with NMF...
#Results: with padj threshold of 0.001, sig genes drops (7086 --> 1626)
#Less severe than NovaSeq platform, but keep consistent. Supported by RIN histograms following removal of glial marker genes only. 
#testsig = res[! is.na(res$padj) & res$padj<0.001,]
# hist(testsig$padj,breaks = 50)
# 
#length(rownames(sig2));length(rownames(testsig))
##########################################################################

coldata2$RIN = scale(coldata2$RIN,center = T)

dds = DESeqDataSetFromMatrix(countData = rALSCountData, colData = coldata2, design= ~ RIN, tidy=F)
dseq = DESeq(dds,betaPrior=T)
res = results(dseq)
sig2 = res[! is.na(res$padj) & res$padj<0.05,]
write.csv(sig2,"HISEQ_SpinalCord_ALSCohort_4Covar_RINSigGenes_9-15-23.csv")
...
write.csv(sig3,"HISEQ_SpinalCord_ALSCohort_4Covar_SiteSigGenes_9-15-23.csv")
...
write.csv(sig4,"HISEQ_SpinalCord_ALSCohort_4Covar_TissueSigGenes_9-15-23.csv")
...
dim(vstcounts);dim(filtdata)

file = "HISEQ_SpinalCord_ALSCohort_4Covariates_VST_Gene_TE_9-15-23.csv"
write.csv(filtdata,file)
file = "HISEQ_SpinalCord_ALSCohort_4Covariates_VST_Gene_TE_9-15-23.txt"
...
file = "HISEQ_SpinalCord_ALSCohort_4Covariates_MAD10k_GeneENSEMBL_TE_9-15-23.txt"
write.table(mad10000,file,sep="\t",quote=F,row.names=T,col.names=T)
file = "HISEQ_SpinalCord_ALSCohort_4Covariates_MAD10k_GeneENSEMBL_TE_9-15-23.csv"
write.csv(mad10000,file)

mad5000 = filtdata[match(names(madval)[1:5000],rownames(filtdata)),] #This gives slightly less than 10k genes
file = "HISEQ_SpinalCord_ALSCohort_4Covariates_MAD5k_GeneENSEMBL_TE_9-15-23.txt"
write.table(mad5000,file,sep="\t",quote=F,row.names=T,col.names=T)
file = "HISEQ_SpinalCord_ALSCohort_4Covariates_MAD5k_GeneENSEMBL_TE_9-15-23.csv"
write.csv(mad5000,file)


#Design independent transformation - validation (confirmed)
# dds = DESeqDataSetFromMatrix(countData = rALSCountData, colData = coldata2, design= ~ Sex + RIN + Site + Tissue, tidy=F)
# dseq = DESeq(dds,betaPrior=T)
# vsd = varianceStabilizingTransformation(dseq)
# vstcounts = assay(vsd)
# filtdata = vstcounts[! (rownames(vstcounts) %in% sig),]
# madval = apply(as.matrix(filtdata),1,mad)
# madval = madval[order(madval,decreasing=T)]
# mad10000 = filtdata[match(names(madval)[1:10000],rownames(filtdata)),]
...
for(i in 1:nrow(MAD10k)){
  ...
  matchind = which(gene_positions$ensembl_gene_id == sub("\\..*","",MAD10k[i,1]))
  if(length(matchind)==1){
    MAD10k[i,1] = gene_positions$hgnc_symbol[matchind]
  ...
  if((i %% 1000) == 0) cat("% Done:",i/nrow(MAD10k)*100,"\n")
}

for(i in 1:nrow(MAD10k)){
  if(MAD10k[i,1] == ""){
    MAD10k[i,1] = rownames(MAD10k)[i]
  }
}
...
file = "HISEQ_SpinalCord_ALSCohort_MAD10k_4Covariates_GeneSYMBOL_TE_9-15-23.csv"
write.csv(MAD10k_SAKE,file)


################## MAD 5k 
MAD5k = mad5000
blank = rep(NA,nrow(MAD5k))
MAD5k = cbind(blank,MAD5k)
MAD5k[,1] = rownames(MAD5k)


ensembl.genes = rownames(MAD5k) 
ensembl_version = "https://apr2020.archive.ensembl.org"
species="human"
ensembl = biomaRt::useMart(biomart = "ENSEMBL_MART_ENSEMBL", dataset = "hsapiens_gene_ensembl", host=ensembl_version)
gene_positions <- biomaRt::getBM(filters = 'ensembl_gene_id',attributes=c('ensembl_gene_id','hgnc_symbol'), values = ensembl.genes, mart = ensembl)
...
#Vectorized for speed - 9/11/23
for(i in 1:nrow(MAD5k)){
  
  matchind = which(gene_positions$ensembl_gene_id == sub("\\..*","",MAD5k[i,1]))
  if(length(matchind)==1){
    MAD5k[i,1] = gene_positions$hgnc_symbol[matchind]
  }else if(length(matchind) > 1){
    ORF1 = grep("^C.orf",gene_positions$hgnc_symbol[matchind])
    ORF2 = grep("^C..orf",gene_positions$hgnc_symbol[matchind])
    ORFind = as.numeric(c(ORF1,ORF2))
  ...
  if((i %% 1000) == 0) cat("% Done:",i/nrow(MAD5k)*100,"\n")
}

for(i in 1:nrow(MAD5k)){
  if(MAD5k[i,1] == ""){
    MAD5k[i,1] = rownames(MAD5k)[i]
  }
}

uniquern = make.names(MAD5k[,1],unique = T)
rownames(MAD5k) = uniquern
MAD5k_SAKE = MAD5k[,-1]


#Order rownames alphabetically
MAD5k_SAKE = MAD5k_SAKE[order(row.names(MAD5k_SAKE)),]
file = "HISEQ_SpinalCord_ALSCohort_MAD5k_4Covariates_GeneSYMBOL_TE_9-15-23.csv"
write.csv(MAD5k_SAKE,file)
